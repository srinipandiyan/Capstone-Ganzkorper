...
/** Creates a history session, updates the db, and returns the new history data.
   ...
   * Returns { id, user_id, exercise_id, weight_used, num_sets, num_reps, created_at }
   **/

static async create(userRef, exerciseId, userInput) {
    const id = uuidv4();
    const { weightUsed, numSets, numReps } = userInput;
    const createdAt = moment().format('MM-DD-YYYY');
    const result = await db.query(
          `INSERT INTO histories (id, user_id, exercise_id, weight_used, num_sets, num_reps, created_at)
           VALUES ($1, $2, $3, $4, $5, $6, $7)
           RETURNING id, user_ref AS "userRef", exercise_id AS "exerciseId", weight_used AS "weightUsed", num_sets AS "numSets", num_reps AS "numReps", created_at AS "createdAt"`,
        [id, userRef, exerciseId, weightUsed, numSets, numReps, createdAt]);
